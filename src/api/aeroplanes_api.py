...
class AeroplanesAPI(BaseAPI):
    ...
    def get_country_coordinates(self, country: str) -> Optional[List[str]]:
        ...
        """
        headers = {"User-Agent": "coursework-aeroplanes/1.0"}
        ...
        try:
            ...
            response = get(url=self.openstreetmap_url, params=params, headers=headers, timeout=30)
    ...
    def get_aeroplanes(self, country: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = get(url=self.opensky_url, params=params, timeout=self.timeout)
            response.raise_for_status()
